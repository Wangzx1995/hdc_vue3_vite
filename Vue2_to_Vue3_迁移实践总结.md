...
为了兼容旧代码中大量 `el-icon-xxx` 字符串图标（如 `el-button icon="el-icon-plus"`），在 `main.js` 中注册了 `el-icon-*` 别名组件：
...
Vue 3 中 `slot` / `slot-scope` 已废弃，统一改为 `v-slot`（`#` 缩写）。

由于项目量大，编写了 `fix-slots.js` 脚本做批量转换，覆盖以下场景：

- `<template slot="xxx">` → `<template #xxx>`
- 普通元素上的 `slot="xxx"` 包裹为 `<template #xxx>`
- `<el-popover slot="reference">` 等场景统一改为 `<template #reference>`
...
后续补充 `resolve.extensions`，避免省略扩展名时解析失败：
...
## 四、Element Plus 与线上样式对齐

### 4.1 样式兼容层 `vue3-compat.less`

Element Plus 默认样式与线上 Element UI 差异较大，新增 `src/assets/less/vue3-compat.less`，在 `main.js` 中全局引入，覆盖：

- 按钮尺寸、padding、hover/active 态
- 表格表头背景、行高、hover 背景
- 表单、分页、下拉菜单、输入框等基础组件
- 修复 HUI 主题导致的按钮 hover 变灰、表格 hover 缺失等问题

### 4.2 样式穿透 `:deep()`

Vue 3 scoped 样式覆盖子组件时，必须使用 `:deep()` 代替 Vue 2 的 `>>>` 或 `/deep/`：
...
### 4.3 Element Plus API 差异批量处理

迁移过程中需统一处理以下 Element Plus API 差异：

| 场景             | Element UI / 旧写法           | Element Plus / 新写法   |
| ---------------- | ----------------------------- | ----------------------- |
| 按钮尺寸         | `size="mini"`                 | `size="small"`          |
| 文字按钮         | `type="link"` / `type="text"` | `type="text"`（统一）   |
| Dialog 可见性    | `:visible.sync="xxx"`         | `v-model="xxx"`         |
| Popover 触发元素 | `slot="reference"`            | `<template #reference>` |
| 输入框 suffix    | `slot="suffix"`               | `<template #suffix>`    |
| Dialog footer    | `slot="footer"`               | `<template #footer>`    |
| 表单 label 位置  | `label-position`              | 保持兼容                |
| 表格 `scope.row` | `slot-scope="scope"`          | `#default="scope"`      |

### 4.4 headbar 修复

顶部导航 `headbar.vue` 存在两类问题：

1. Vue 3 中无指令的 `<template>` 包装会导致内容不可见，通过 `fix-headbar-template.cjs` 移除多余 `<template>`
2. Element Plus 弹出菜单背景样式失效，通过 `fix-headbar-style.cjs` 追加全局覆盖样式

---

## 五、迁移过程中遇到的坑和难点

### 5.1 `require is not defined`

**现象**：浏览器控制台报 `ReferenceError: require is not defined`。
**根因**：Vite 不支持浏览器侧 `require()`。
**解决**：统一改为 ES Module `import`，包括 JS 库、SCSS、图片、动态路由等。

### 5.2 组件导入缺失 `.vue` 扩展名导致白屏
...
### 5.4 Vue Router 4 路由 name 唯一性校验
...
### 5.7 Element Plus 图标系统迁移工作量大

**现象**：大量图标不显示。
**根因**：Element UI 是字体图标，Element Plus 是 SVG 图标。
**解决**：

- 优先通过 `main.js` 注册 `el-icon-*` 别名，兼容旧代码中的字符串图标
- 对新代码统一使用 `<el-icon><XXX /></el-icon>`
- 批量替换 `<i class="el-icon-xxx">` → `<el-icon><XXX /></el-icon>`
...
### 5.11 资源路径与 Sass 弃用警告

**现象**：构建日志出现 public 资源引用错误、Dart Sass `@import` 和 legacy JS API 弃用警告。
**根因**：

- 代码中通过 JS `import` 引用了 `static/` 下的 public 资源
- 大量 `.less` / `.scss` 仍使用 `@import`，且部分 Sass 函数写法已废弃
- HTML 中存在 `<div>` 作为 `<tr>` 子元素等结构性错误

**解决**：

- 将 public 资源引用改为根路径方式，不在 JS 中 `import` public 目录文件
- 批量替换 `str-index` 为 `string.index` 等兼容写法
- 修复 `<table>` 结构错误
- 更新构建日志，增强警告可读性

---

## 六、Webpack 迁移到 Vite 的优点
...
6. **保留兼容层**：对于 `process.env`、旧 UI 组件、第三方库、Element Plus 图标等，优先通过兼容层或别名解决，减少业务代码改动。
7. **批量脚本辅助**：对于插槽语法、图标引用、路径扩展名等重复性改动，编写脚本批量处理，避免人工遗漏。
8. **样式回归测试**：Element Plus 样式差异大，需逐页对比线上效果，及时补充 `vue3-compat.less` 覆盖。

---

## 八、总结

本次迁移的核心思路是 **"先跑起来，再逐个修复"**。Vue 2 到 Vue 3 的改动主要集中在全局 API、路由、状态管理、UI 库和移除特性上；Webpack 到 Vite 的改动则更多是构建范式从 CommonJS/bundle 到 ESM/按需编译的转变。最耗时、最容易踩坑的地方往往不是框架本身的语法变化，而是 **大量业务代码中的 `require()` 替换、`.vue` 扩展名补全、路由循环依赖解耦、环境变量兼容、Element Plus 图标和 hui-pro 组件的适配、以及样式层的逐页对齐**。

通过保留 Options API、添加兼容层、编写批量迁移脚本，显著降低了迁移成本。后续仍需持续关注 Sass 弃用警告、第三方库兼容性以及样式回归问题。
